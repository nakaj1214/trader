...
これはClaude Codeのエージェント生成機能で使用される正確なシステムプロンプトであり、広範な本番使用を通じて洗練されたものです。
...
このプロンプトを使用してエージェント設定を生成します：

```markdown
**ユーザー入力:** "I need an agent that reviews pull requests for code quality issues"

**上記のシステムプロンプトと共にClaudeに送信：**
...
JSON出力を取得してエージェントのMarkdownファイルを作成します：
...
ベースプロンプトは優れていますが、特定のニーズに合わせて強化できます：

**セキュリティ重視のエージェント用：**
```
「Architect Comprehensive Instructions」の後に追加：
...
**テスト生成エージェント用：**
```
「Optimize for Performance」の後に追加：
...
プロンプトはCLAUDE.mdコンテキストの使用を明示的に言及しています：
- エージェントはプロジェクトパターンに準拠すべき
- プロジェクト固有のコーディング標準に従うべき
- 確立されたプラクティスを尊重すべき

### 2. プロアクティブなエージェント設計

プロアクティブな使用例を示す例を含めます：
```
<example>
...
コードレビューエージェントの場合、コードベース全体ではなく「最近書かれたコード」を前提とします：
```
...
システムプロンプトには常に明確な出力フォーマットを定義します：
```
**Output Format:**
Provide results as:
...
プラグイン用のエージェントを作成する際にこのシステムプロンプトを使用します：

1. エージェント機能に関するユーザーリクエストを受け取る
2. このシステムプロンプトと共にClaudeに送信する
3. JSON出力（identifier, whenToUse, systemPrompt）を取得する
4. フロントマター付きのエージェントMarkdownファイルに変換する
5. エージェントバリデーションルールで検証する
6. トリガー条件をテストする
7. プラグインの `agents/` ディレクトリに追加する

これにより、Claude Codeの内部実装から実績のあるパターンに従ったAI支援エージェント生成が実現できます。
