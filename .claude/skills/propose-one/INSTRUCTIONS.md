...
**ユーザーの選択後:**
- **1 を選んだ場合**: `/verify-before-fix` スキルを起動し、`propose-one` を終了する
- **2 を選んだ場合**: Step 2（create-proposal）に進む
...
proposal.md 生成後に**必ず停止する**。create-plan は自動実行しない。

```
proposal.md を作成しました。確認後、`/create-plan` で計画書の作成を開始してください。
```
