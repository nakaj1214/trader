...
from src.screening.filters import FilterChain, GoldenCrossFilter, LiquidityFilter, MarketCapFilter
...
class TestUniverse:
    ...
    def test_load_nonexistent(self) -> None:
        assert load_universe("nonexistent_market") == []
...
class TestMarketCapFilter:
    def test_filters_below_threshold(self) -> None:
        df = pd.DataFrame({"ticker": ["A", "B", "C"], "market_cap": [5e9, 15e9, None]})
        result = MarketCapFilter().apply(df, {"filters": {"market_cap": {"min": 10e9}}})
        assert len(result) == 2

    def test_zero_threshold_passes_all(self) -> None:
        df = pd.DataFrame({"ticker": ["A"], "market_cap": [1.0]})
        assert len(MarketCapFilter().apply(df, {"filters": {"market_cap": {"min": 0}}})) == 1


class TestLiquidityFilter:
    def test_filters_illiquid(self) -> None:
        df = pd.DataFrame({"ticker": ["AAPL", "TINY"], "avg_dollar_volume": [5_000_000, 100]})
        result = LiquidityFilter().apply(
            df,
            {"filters": {"liquidity": {"min_dollar_volume_us": 1_000_000, "min_dollar_volume_jp": 0}}},
        )
        assert len(result) == 1
...
class TestGoldenCrossFilter:
    def test_filters_death_cross(self) -> None:
        df = pd.DataFrame({"ticker": ["A", "B", "C"], "golden_cross": [1.0, 0.0, None]})
        result = GoldenCrossFilter().apply(df, {"filters": {"golden_cross": {"enabled": True}}})
        assert len(result) == 2

    def test_disabled_passes_all(self) -> None:
        df = pd.DataFrame({"ticker": ["A"], "golden_cross": [0.0]})
        assert len(GoldenCrossFilter().apply(df, {"filters": {"golden_cross": {"enabled": False}}})) == 1
...
class TestScorer:
    def test_extracts_singleton_ticker_multiindex(self) -> None:
        columns = pd.MultiIndex.from_tuples(
            [("Close", "7203.T"), ("Volume", "7203.T")], names=["Price", "Ticker"]
        )
    ...
    def test_select_top_n(self) -> None:
        assert select_top_n(pd.Series({"A": 0.9, "B": 0.7, "C": 0.8}), 2) == ["A", "C"]
