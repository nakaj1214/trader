...
(function () {
  function renderMacroBanner(macro) {
    ...
    if (series.FEDFUNDS && series.FEDFUNDS.latest_value != null) {
      parts.push("米国金利: " + series.FEDFUNDS.latest_value.toFixed(2) + series.FEDFUNDS.unit);
    }
    if (series.T10Y2Y && series.T10Y2Y.latest_value != null) {
      var spread = series.T10Y2Y.latest_value;
      parts.push("金利差: " + (spread >= 0 ? "+" : "") + spread.toFixed(2) + series.T10Y2Y.unit);
    }
    ...
    var asOf = macro.data_as_of_utc ? "データ日: " + macro.data_as_of_utc.slice(0, 10) : "";

    banner.className = "macro-banner" + (isRiskOff ? " risk-off" : "");
    banner.style.display = "";
    banner.innerHTML =
      '<div class="macro-banner-inner">' +
      '<span class="macro-banner-label">' + (isRiskOff ? "⚠ 市場が不安定です" : "市場の状況") + "</span>" +
