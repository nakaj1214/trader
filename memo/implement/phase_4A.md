...
Trade Ledger、`position_id`、pending payload、tombstone、実現損益、MFE/MAE、workflow直列化（`position_id`ベースの単一writer保証等）は対象外とする。REQ-025の状態キーは既存入力だけで作れる`(ticker, entry_date)`とし、同じキーの複数保有はfail-closedにする。同日・同一tickerの複数positionを個別管理する必要が生じた時点で、REQ-027と一緒に`position_id`を導入する。

ただし例外として、`.github/workflows/position_monitor.yml`の`concurrency.cancel-in-progress: true`だけは今回`false`に変更する（本文参照）。これは`position_id`等の大掛かりなworkflow直列化とは別の、既存workflowの1行設定変更で防げる具体的な重複通知シナリオへの対応であり、対象外にする理由がない。
...
- entry時刻を持たないため、`entry_date == evaluated_at.date()`では当日バーによるStop判定を行わず、HWMは分割調整後entry priceのままとする。**この設計により、entry日に付けた高値（約定前後を区別できないため）は翌営業日以降も恒久的にHWMへ反映されない**（日足履歴フィルタが`row_date > entry_date`のため、entry日自身の行は将来にわたって二度と`daily_highs`に含まれない）。例えば100円で約定後に150円まで上昇し、翌日120円で寄り付いても、判定時のHWMは100円、15% stopは85円のため、本来の127.5円水準での通知は発生しない。そのバーの判定後にHWMは120円、stopは102円へ更新される。**`entry_at`（時刻）を新たに入力させて約定後バーだけを使う設計にはしない**（「保有銘柄」への入力項目を増やすコストが見合わないため）。この制約は仕様・README・受入テストに明記し、黙って許容するのではなく既知の制約として運用者に周知する。`[ponytail: entry日の高値はHWMに永久反映されない既知の制約。約定時刻入力を追加すれば解消できるが優先度は低いと判断]`
...
- entry日に100→150円と推移し、翌営業日に120円で寄り付く合成データで、entry日の150円がHWMに反映されずStopが発火しないこと、そのバーの判定後はHWMが120円へ更新されることを確認する（意図した制約の回帰テスト）。
...
- `STALE_QUOTE_THRESHOLD_MINUTES_IN_SESSION = 10`と`STALE_QUOTE_THRESHOLD_MINUTES_OUTSIDE_SESSION = 60`を定義する。**10分は実測に基づく値ではなく暫定値**である（proposalはyfinanceの実際の遅延特性を確認して5分か10分かを決める前提だったが、実運用データが無いため確認できていない）。定数定義の直上にその旨をコメントで残し、READMEにも「10分は暫定値。運用開始後にyfinanceの実際の遅延を観察し、(a) 実測が常に5分以内に収まるなら5分へ厳格化する、(b) 正常な業務上のprovider遅延（stale化した原因が実際の障害ではなく通常のデータ反映ラグ）でstale判定が頻発するなら10分より長い値（例: 15〜20分）へ緩和する、のいずれかで見直す」という、観測結果に応じて逆方向にもなり得る再評価条件を明記する（閾値を一律「短くする」方向にしない）。
...
- **`.github/workflows/position_monitor.yml`の`concurrency.cancel-in-progress`を`true`から`false`に変更する。**現行`true`のままだと、Slack送信成功後・status書き込み完了前に次のscheduled/manual実行が始まった場合、先行runがキャンセルされて`last_notified_at`の保存が失われ、次のrunが未通知状態のまま同じalertを再送してしまう（workflow直列化そのものは対象外だが、この特定の重複シナリオは`cancel-in-progress`の1行変更だけで防げるため対応する）。`false`にすると後続runは先行runの完了を待ってから開始する。
...
- `position_monitor.yml`の`concurrency.cancel-in-progress`が`false`になっていることを確認する（設定ファイルの静的チェック）。
