...
MANAGED_BEGIN = "<!-- adaptive-codex-harness-v6.4:begin -->"
MANAGED_END = "<!-- adaptive-codex-harness-v6.4:end -->"
OLD_MANAGED_BLOCKS = (
...
trait HarnessSideEffectIsolation
{
    protected function harnessEnableSideEffectIsolation(): void
    {
        ...
        $storageRoot = rtrim(sys_get_temp_dir(), DIRECTORY_SEPARATOR)
            .DIRECTORY_SEPARATOR.'adaptive-codex-harness'
...
RULES_TEXT = r'''# Adaptive Codex Harness v6.4 side-effect safety rules.
...
prefix_rule(
    ...
    decision = "forbidden",
    justification = "Harness v6.4 forbids destructive database reset/wipe commands from Codex."
)

prefix_rule(
    pattern = ["php", "artisan", ["queue:work", "queue:listen", "schedule:run", "schedule:work", "horizon"]],
    decision = "forbidden",
    justification = "Harness v6.4 forbids starting workers/schedulers as autonomous verification because they may perform persistent external work."
)

prefix_rule(
    pattern = ["git", ["clean", "reset"]],
    decision = "prompt",
    justification = "Harness v6.4 treats Git cleanup/reset as potentially destructive; prefer scoped reversible operations."
)
'''

VERIFY_PREFLIGHT = r'''# adaptive-codex-harness-v6.4-side-effect-preflight:begin
...
class Txn:
    def __init__(self, root: Path, dry_run: bool):
        ...
        self.backup = root / ".codex-harness-backup" / f"v6.4-{time.strftime('%Y%m%d-%H%M%S')}"
...
def patch_skills(root: Path, txn: Txn) -> None:
    targets = [
        (root / ".agents/skills/harness-develop/SKILL.md", "### v6.4 safety / bounded execution", DEVELOP_APPEND),
        (root / ".agents/skills/harness-debug/SKILL.md", "### v6.4 debug safety", DEBUG_APPEND),
        (root / ".agents/skills/harness-verify/SKILL.md", "### v6.4 verification policy", VERIFY_APPEND),
        (root / ".agents/skills/harness-review/SKILL.md", "### v6.4 reviewer policy", REVIEW_APPEND),
        (root / ".agents/skills/maintain-project-knowledge/SKILL.md", "### v6.4 knowledge policy", KNOWLEDGE_APPEND),
    ]
...
def patch_old_feedback_text(root: Path, txn: Txn) -> int:
    changed = 0; replacements = {
        ...
        "Adaptive Harness v6.3 advisory quality policy:": "Adaptive Harness v6.4 advisory quality policy:",
        "Adaptive Harness v6.1 scoped quality gate:": "Adaptive Harness v6.4 advisory quality policy:",
        "Adaptive Harness v6 quality gate:": "Adaptive Harness v6.4 advisory quality policy:",
    }
    for base in (root / ".codex", root / ".agents"):
        if not base.exists(): continue
        for p in base.rglob("*"):
...
def patch_verify_preflight(root: Path, txn: Txn) -> bool:
    ...
    text = path.read_text(encoding="utf-8", errors="replace")
    if "adaptive-codex-harness-v6.4-side-effect-preflight:begin" in text:
        return False

    # Remove earlier Harness preflight blocks before installing the v6.4 combined guard.
    text = re.sub(
...
def write_v64_files(root: Path, txn: Txn) -> None:
    txn.write_json(root / ".codex/harness/v6_4_policy.json", POLICY)
    ...
    if old_rule.exists():
        ...
        if "Adaptive Codex Harness v6.3" in old_text or "Adaptive Codex Harness v6.4 DB safety" in old_text:
            txn.write_text(old_rule, "# Superseded by harness-side-effect-safety.rules in Adaptive Codex Harness v6.4.\n")

    notes = """# Adaptive Codex Harness v6.4

v6.4 extends the v6.3 fail-closed DB guard into fail-closed side-effect isolation.
...
- storage_path() is redirected to a temporary test storage root
...
"""
    txn.write_text(root / ".codex/harness/V6_4.md", notes)
...
def self_check(root: Path) -> list[str]:
    ...
    if not vf.exists() or vf.read_text(encoding="utf-8", errors="replace").strip() != VERSION:
        problems.append("VERSION is not 6.4.0")
    ...
    if not (root / ".codex/harness/v6_4_policy.json").exists():
        problems.append("missing v6_4_policy.json")
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Adaptive Codex Harness v6/v6.1/v6.2/v6.3 -> v6.4 fail-closed side-effect safety installer/upgrader"
    )
    ...
    if not already and source is None:
        raise SystemExit(
            ...
            "v6配布フォルダ内へこのinstall_v6_4.pyを置いて実行してください。"
        )
    ...
    try:
        ...
        if not already or args.refresh_base:
            ...
            print("[BASE] existing harness preserved; applying v6.4 delta only")
        ...
        print(f"[POLICY] stale v6/v6.1/v6.3 gate text updated in {stale} files")

        patch_agents(root, txn)
        patch_skills(root, txn)
        write_v64_files(root, txn)
    ...
    print("  - Test storage_path() is redirected to a temporary directory.")
    ...
    if not all_safe:
        print("\n[IMPORTANT] Harness v6.4 is installed, but tests are intentionally fail-closed until both guards PASS.")
    return 0
