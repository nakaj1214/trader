    ...
    if gate.get("enabled", True) and changed:
        ...
        if gate.get("require_verification", True) and needs_verification(cfg):
            ...
            if stamp.get("state_hash") != current or not stamp.get("passed"):
                missing.append("run `python3 .codex/harness/scripts/verify.py` and resolve required failures")
        if gate.get("require_independent_review", True) and needs_review(cfg):
            ...
            if stamp.get("state_hash") != current:
                missing.append("spawn one bounded independent `reviewer`, address blocking findings, and obtain `VERDICT: PASS`")
        if missing:
            ...
            if count < max_blocks:
                ...
                return {"decision": "block", "reason": "Adaptive Harness v6 quality gate: " + "; ".join(missing) + ". Later edits invalidate the relevant stamp."}
            return {"continue": True, "suppressOutput": True}
